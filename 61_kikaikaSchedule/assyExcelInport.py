...
def pick_best_file_by_period(folder: str, must_tokens, start_date: date, end_date: date,
    ...
    2) 指定期間内の行数が最大のファイルを採用
    3) 同点なら更新日時(mtime)が新しい方
    """
...
def run() -> str:
    ...
    file_paths = [aw_path, cp_path, fx_path, hc_path, WEB_XLSM]
    ...
    # 既存：return combined_file_path
    return {
        "combined": combined_file_path,
        "picked_files": file_paths,  # ← 追加：採用したフルパス一覧
    }
